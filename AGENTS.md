...
- Drawing writes race Obsidian itself, not just other MCP callers. An open Excalidraw view autosaves every 60 seconds from its own in-memory scene and can overwrite an external write; `withPathLock` and `atomicWriteFile` do not defend against that. `base_version` (via `versionOf` over the whole file text) is the honest mitigation — it converts the race into a rejected write instead of a lost one — and the tool descriptions tell agents to close the drawing in Obsidian first.
